...
const execute = async (CLI_KEYS, CLI_ARGS) => {
  try {
    console.log("path ====>", path.resolve(process.cwd(), `package.json`));
    ...
    console.log("CLI_ARGS ====>", CLI_ARGS[2]);
    const COPY_TEMP_SCRIPT = "";
    const LIB_NAME = CLI_ARGS[0];
    const LIB_PATH = `src/library/${LIB_NAME}`;
    if (!fs.existsSync(LIB_PATH)) {
      fs.mkdirSync(LIB_PATH);
    }
    const LIB_FILE_NAME = splitString[1];
    ...
    fs.renameSync(
      `${path.resolve(
        process.cwd(),
        `/node_modules/@njs2/base/template/libraryStructure/`+CLI_ARGS[2]+`/`+CLI_ARGS[2]+`.lib.js`
      )}`,
